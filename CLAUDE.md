...
- **`Iconfilegroup`** — groups `Iconfilesubgroup` instances under a directory; owns `collect` (recursive filesystem scan via `collect_sub`) and `analyze`; `@iconfiles_hs` keyed by `[category]`, `[category, l1]`, or `[category, l1, l2]` arrays
...
- **`Dump`** — wraps `PStore` for binary caching of scan results between runs; skipped when dump file is absent or `-x` is passed
...
- **`Cmd`** (Thor subclass in `lib/cpiconfiles/cmd.rb`) — defines subcommands and options; delegates to `Cli`
...
- **Ruby 4.0** is in use (Windows, `C:/Ruby40-x64`). Several gems that were previously stdlib must be declared explicitly in the Gemfile: `logger`, `csv`, `ostruct`, `pstore`
- **RuboCop**: single quotes enforced (`Style/StringLiterals`), max line length 120; plugins: `rubocop-rake`, `rubocop-rspec`; see `.rubocop.yml` / `.rubocop_todo.yml`
- **`frozen_string_literal`**: `Style/FrozenStringLiteralComment` is disabled
- **RSpec**: `expect` syntax only (`config.disable_monkey_patching!`); tag tests with symbols (e.g., `:yaml => true`); fixture data in `spec/test_data/`
- **CSV format**: columns are `kind,icon_size,category,basename,pathn` (no header row)
- **`Iconlist#valid?`** returns `true` in `:BEFORE_LOAD` state, so `collect` is only skipped after a successful dump load (`move_state` transitions the state)
- **`p` statements** remain in production code (e.g., `cmd.rb`) — intentional debug output
